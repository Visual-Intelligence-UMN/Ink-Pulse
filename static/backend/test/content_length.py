...
def convert_format():
    script_dir = os.path.dirname(os.path.abspath(__file__))
    static_dir = os.path.dirname(script_dir)
    note_dir = os.path.join(static_dir, "chi2022-coauthor-v1.0", f"length-{dataset_name}.json")
    ...
    save_path = os.path.join(static_dir, "chi2022-coauthor-v1.0", f"length_summary-{dataset_name}.json")
    with open(save_path, "w", encoding="utf-8") as f:
