...
def process_files(base_path):
    for filename in os.listdir(base_path):
        if filename.endswith('.jsonl'):
            ...
            read_file(file_path)
            print(f"Updated time in file: {filename}")

# Delete sentences with fewer than 5 changed characters.
...
def filter_and_save_json(json_path, delta=5):
    ...
    print(f"Remove lines in file: {json_path}")

def main(dataset_name):
    script_dir = os.path.dirname(os.path.abspath(__file__)) 
    static_dir = os.path.dirname(os.path.dirname(script_dir))
    json_dir = os.path.join(static_dir, f"dataset/{dataset_name}/coauthor-sentence")
    process_files(json_dir)
